...
export const PluginShapes = () => {
  const createShape = (id, name) => ({ id, name, mode: 'preset', shapeVal: 'square', color: '#8b5cf6', nodes: P_NODES['square'], rounded: 0, x: 100, y: 50, w: 200, h: 200, visible: true, locked: false });
...
// =========================================================================
// 3. PIXEL DRAWING (FIX MATRIKS: BISA GAMBAR SAAT ZOOM & ROTATE)
// =========================================================================
...
// FIX NAMA BUG: Dari PluginPixelArt kembali menjadi PluginPixelDrawing agar page.jsx tidak crash!
export const PluginPixelDrawing = () => {
  ...
  // FIX MATH UTAMA: Menghitung posisi sentuhan dengan membalikkan nilai matriks rotasi & scale
  const paintByEvent = (e) => {
    ...
    // Balikkan Rotasi Jari (Inverse Matrix)
    const angleRad = -rotation * (Math.PI / 180);
    const rotatedX = dx * Math.cos(angleRad) - dy * Math.sin(angleRad);
    const rotatedY = dx * Math.sin(angleRad) + dy * Math.cos(angleRad);

    const xReal = (rotatedX / scale) + ((gridSize * pixelSizePx) / 2);
    const yReal = (rotatedY / scale) + ((gridSize * pixelSizePx) / 2);
