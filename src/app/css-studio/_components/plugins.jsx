...
export const PluginShapes = () => {
  ...
  const duplicateShape = (id) => {
    const toCopy = shapes.find(s => s.id === id);
    if (!toCopy) return;
    const newId = Date.now();
    setShapes([...shapes, { ...toCopy, id: newId, name: `${toCopy.name} Copy`, x: toCopy.x + 20, y: toCopy.y + 20 }]);
    setActiveShapeId(newId);
  };
  const deleteShape = (id) => {
    ...
    setShapes(newShapes);
    if (activeShapeId === id) setActiveShapeId(newShapes[newShapes.length - 1].id);
  };
  ...
  const handlePointerDownShape = (e, id, shapeState) => {
    if (activeTool === 'pan') {
      e.stopPropagation();
      setActiveShapeId(id);
      if (!shapeState.locked) {
         setDraggingShape(id); setDragStart({ x: e.clientX, y: e.clientY }); setElemStart({ x: shapeState.x, y: shapeState.y });
          ...
          {shapes.map((s) => {
            ...
            return (
              <div 
                ...
                className={`absolute ${activeTool === 'pan' ? (isActive ? 'cursor-grabbing ring-1 ring-cyan-500' : (s.locked ? '' : 'cursor-grab hover:ring-1 ring-white/30')) : 'pointer-events-none'}`}
    ...
    <>
      ...
      <div className="border-t border-[#1f1f1f] pt-4 pb-2 mb-4">
         <div className="flex items-center justify-between mb-3">
            ...
            <button onClick={addShapeLayer} className="text-[8px] text-cyan-400 bg-cyan-500/10 border border-cyan-500/30 px-2 py-1 rounded uppercase tracking-widest hover:bg-cyan-500/20">+ Shape Baru</button>
         </div>
         <div className="space-y-2 max-h-[150px] overflow-y-auto custom-scroll pr-1">
            {[...shapes].reverse().map(s => (
               <div key={s.id} onClick={() => setActiveShapeId(s.id)} className={`flex items-center justify-between p-2 rounded-lg border cursor-pointer transition-all ${activeShapeId === s.id ? 'bg-[#1a1a1a] border-cyan-500' : 'bg-[#0a0a0a] border-[#2a2a2a] hover:border-[#444]'}`}>
                  <div className="flex items-center gap-3">
                     ...
                     <span className={`text-[11px] font-bold ${activeShapeId === s.id ? 'text-white' : 'text-slate-400'}`}>{s.name}</span>
                  </div>
      ...
      <div className="flex bg-[#0a0a0a] p-1 rounded-lg border border-[#2a2a2a] w-full mb-4">
        ...
        <button onClick={() => {updateActive('mode', 'custom'); setActiveTool('pen');}} className={`flex-1 py-2 rounded-md text-[9px] font-bold uppercase transition-all ${activeShape?.mode === 'custom' ? 'bg-cyan-500/10 text-cyan-400 border border-cyan-500/30' : 'text-slate-500 hover:text-slate-300'}`}>Custom Pen</button>
      </div>
...
export const PluginPixelDrawing = () => {
  ...
  const controls = (
    <>
      ...
      <div className="flex items-center gap-3 mb-3 border-t border-[#1f1f1f] pt-4">
