...
import { PluginTip, FigmaSlider, FigmaColorPicker, FigmaToggle } from './ui';

// Ikon In-line khusus Pixel Draw agar 100% aman
const PixIcons = {
...
export const PluginPixelDrawing = () => {
  ...
  // NAVIGASI MOBILE (Tampilan Native)
  ...
  // Mencegah Scroll Bawaan HP saat Fullscreen atau Menggambar
  useEffect(() => {
  ...
  const paintByCoords = (clientX, clientY) => {
    ...
    const unscaledX = (dx / actualScale) + ((gridSize * pixelSizePx) / 2);
    const unscaledY = (dy / actualScale) + ((gridSize * pixelSizePx) / 2);
  ...
  const handleTouchStart = (e) => {
    if (activeTool === 'pan' || e.touches.length > 1) {
      if (e.touches.length === 2) {
        ...
        touchRef.current.panX = pan.x; touchRef.current.panY = pan.y;
        touchRef.current.scale = scale;
      }
      return;
    }
    setIsDrawing(true);
    paintByCoords(e.touches[0].clientX, e.touches[0].clientY);
  };
  ...
  const handleTouchMove = (e) => {
    ...
    if (isDrawing) paintByCoords(e.touches[0].clientX, e.touches[0].clientY);
  };
  ...
  const ToolsTab = () => (
    <div className="grid grid-cols-3 gap-3 animate-fade-in-fast h-full items-start">
      <button onClick={() => setActiveTool('draw')} className={`flex flex-col items-center justify-center gap-2 p-4 rounded-2xl transition-all border ${activeTool === 'draw' ? 'bg-cyan-500/10 text-cyan-400 border-cyan-500 shadow-[0_0_15px_rgba(6,182,212,0.2)]' : 'bg-[#141414] text-slate-400 border-transparent hover:bg-[#1f1f1f]'}`}><div className="w-6 h-6"><PixIcons.Brush /></div><span className="text-[10px] font-black tracking-widest uppercase">Kuas</span></button>
      <button onClick={() => setActiveTool('erase')} className={`flex flex-col items-center justify-center gap-2 p-4 rounded-2xl transition-all border ${activeTool === 'erase' ? 'bg-cyan-500/10 text-cyan-400 border-cyan-500 shadow-[0_0_15px_rgba(6,182,212,0.2)]' : 'bg-[#141414] text-slate-400 border-transparent hover:bg-[#1f1f1f]'}`}><div className="w-6 h-6"><PixIcons.Eraser /></div><span className="text-[10px] font-black tracking-widest uppercase">Hapus</span></button>
      <button onClick={() => setActiveTool('bucket')} className={`flex flex-col items-center justify-center gap-2 p-4 rounded-2xl transition-all border ${activeTool === 'bucket' ? 'bg-cyan-500/10 text-cyan-400 border-cyan-500 shadow-[0_0_15px_rgba(6,182,212,0.2)]' : 'bg-[#141414] text-slate-400 border-transparent hover:bg-[#1f1f1f]'}`}><div className="w-6 h-6"><PixIcons.Bucket /></div><span className="text-[10px] font-black tracking-widest uppercase">Isi</span></button>
      <button onClick={() => setActiveTool('picker')} className={`flex flex-col items-center justify-center gap-2 p-4 rounded-2xl transition-all border ${activeTool === 'picker' ? 'bg-cyan-500/10 text-cyan-400 border-cyan-500 shadow-[0_0_15px_rgba(6,182,212,0.2)]' : 'bg-[#141414] text-slate-400 border-transparent hover:bg-[#1f1f1f]'}`}><div className="w-6 h-6"><PixIcons.Picker /></div><span className="text-[10px] font-black tracking-widest uppercase">Ambil</span></button>
      <button onClick={() => setActiveTool('pan')} className={`flex flex-col items-center justify-center gap-2 p-4 rounded-2xl transition-all border ${activeTool === 'pan' ? 'bg-amber-500/10 text-amber-500 border-amber-500 shadow-[0_0_15px_rgba(245,158,11,0.2)]' : 'bg-[#141414] text-slate-400 border-transparent hover:bg-[#1f1f1f]'}`}><div className="w-6 h-6"><PixIcons.HandPan /></div><span className="text-[10px] font-black tracking-widest uppercase">Geser</span></button>
      <button onClick={() => { setScale(1); setPan({x:0, y:0}); }} className={`flex flex-col items-center justify-center gap-2 p-4 rounded-2xl transition-all border bg-[#141414] border-transparent text-slate-400 hover:bg-[#1f1f1f] hover:text-white active:scale-95`}><div className="w-6 h-6"><PixIcons.Focus /></div><span className="text-[10px] font-black tracking-widest uppercase">Fokus</span></button>
    </div>
  );
  ...
  const ColorsTab = () => (
    <div className="space-y-4 animate-fade-in-fast">
      ...
      <div className="flex flex-wrap gap-3 mt-4">
        ...
        <button onClick={() => !palette.includes(color) && setPalette([color, ...palette].slice(0, 15))} className="w-10 h-10 rounded-xl border-2 border-[#333] flex items-center justify-center shrink-0 text-slate-500 hover:text-white hover:border-[#555] bg-[#141414] transition-all"><div className="w-4 h-4"><Icons.Plus /></div></button>
      </div>
    </div>
  );
  ...
  const CanvasRenderer = () => (
     <div className="flex-1 relative w-full h-full flex items-center justify-center overflow-hidden bg-[#050505] touch-none"
     ...
     >
        {/* Tombol Undo/Redo di dalam kanvas agar tidak menimpa tools lain */}
        <div className="absolute top-4 right-4 flex gap-2 z-20" onPointerDown={stopProp} onClick={stopProp}>
          <button onClick={handleUndo} disabled={step <= 0} className="w-11 h-11 flex items-center justify-center rounded-xl border border-[#2a2a2a] bg-[#141414]/90 backdrop-blur text-slate-300 disabled:opacity-30 shadow-lg hover:text-white transition-colors active:scale-95"><div className="w-5 h-5"><PixIcons.Undo /></div></button>
          <button onClick={handleRedo} disabled={step >= history.length - 1} className="w-11 h-11 flex items-center justify-center rounded-xl border border-[#2a2a2a] bg-[#141414]/90 backdrop-blur text-slate-300 disabled:opacity-30 shadow-lg hover:text-white transition-colors active:scale-95"><div className="w-5 h-5"><PixIcons.Redo /></div></button>
        </div>

        <div style={{ transform: `translate(${pan.x}px, ${pan.y}px) scale(${scale * baseScale})`, transition: isDrawing ? 'none' : 'transform 0.1s ease-out' }} className="absolute">
          ...
          <div ref={gridRef} className="grid shadow-[0_0_50px_rgba(0,0,0,0.8)] border-t-[4px] border-t-red-500" 
            ...
            {mergedPixels.map((bg, i) => (
              // FIX GRID COLOR MUTLAK: Menggunakan box-shadow inset kelabu
  ...
  return (
    <div className="w-full h-full flex flex-col lg:flex-row bg-[#050505] overflow-hidden absolute inset-0 z-10 font-sans">
       
       {/* 1. BAGIAN ATAS / KIRI: KANVAS GAMBAR MURNI */}
       <div className="flex-[55%] lg:flex-[2] relative flex flex-col border-b lg:border-b-0 lg:border-r border-[#1f1f1f] bg-[#000] overflow-hidden shadow-xl z-20">
          {/* Tombol Fullscreen di Sudut Kiri Atas */}
          <div className="absolute top-4 left-4 z-20 lg:hidden" onPointerDown={stopProp} onClick={stopProp}>
             <button onClick={() => setIsFullscreen(!isFullscreen)} className={`px-4 py-2.5 flex items-center justify-center gap-2 rounded-xl transition-all font-black text-[10px] uppercase tracking-widest shadow-lg active:scale-95 ${isFullscreen ? 'bg-amber-500 text-black border border-amber-400' : 'bg-[#141414]/90 backdrop-blur border border-[#2a2a2a] text-cyan-400'}`}>
                <div className="w-4 h-4">{isFullscreen ? <PixIcons.Close /> : <PixIcons.Expand />}</div> {isFullscreen ? 'Tutup Penuh' : 'Layar Penuh'}
             </button>
          </div>
       ...
       {/* 2. BAGIAN BAWAH / KANAN: PANEL ALAT (MOBILE LACI, DESKTOP SIDEBAR) */}
       <div className={`${isFullscreen ? 'hidden' : 'flex'} flex-[45%] lg:flex-none flex-col bg-[#050505] z-40 shrink-0 lg:w-[400px] lg:h-full`}>
          ...
          <div className="flex-1 overflow-y-auto custom-scroll p-5 sm:p-6 lg:p-7 relative bg-[#0a0a0a]">
             {mobileTab === 'tools' && <ToolsTab />}
             {mobileTab === 'colors' && <ColorsTab />}
             {mobileTab === 'layers' && <LayersTab />}
             {mobileTab === 'settings' && <CanvasTab />}
          </div>

          {/* BOTTOM TAB NAVIGASI MOBILE LENGKAP */}
          <div className="lg:hidden h-[70px] sm:h-20 bg-[#050505] border-t border-[#1a1a1a] flex shrink-0 pb-safe shadow-[0_-10px_20px_rgba(0,0,0,0.5)]">
             <button onClick={() => setMobileTab('tools')} className={`flex-1 flex flex-col items-center justify-center gap-1.5 transition-all ${mobileTab === 'tools' ? 'text-cyan-400 border-t-2 border-cyan-500' : 'text-slate-500 hover:text-slate-300 border-t-2 border-transparent'}`}><div className="w-5 h-5 sm:w-6 sm:h-6"><PixIcons.Brush /></div><span className="text-[9px] sm:text-[10px] font-black uppercase tracking-wider">Alat</span></button>
             <button onClick={() => setMobileTab('colors')} className={`flex-1 flex flex-col items-center justify-center gap-1.5 transition-all ${mobileTab === 'colors' ? 'text-cyan-400 border-t-2 border-cyan-500' : 'text-slate-500 hover:text-slate-300 border-t-2 border-transparent'}`}><div className="w-5 h-5 sm:w-6 sm:h-6"><PixIcons.Picker /></div><span className="text-[9px] sm:text-[10px] font-black uppercase tracking-wider">Warna</span></button>
             <button onClick={() => setMobileTab('layers')} className={`flex-1 flex flex-col items-center justify-center gap-1.5 transition-all ${mobileTab === 'layers' ? 'text-cyan-400 border-t-2 border-cyan-500' : 'text-slate-500 hover:text-slate-300 border-t-2 border-transparent'}`}><div className="w-5 h-5 sm:w-6 sm:h-6"><PixIcons.Layers /></div><span className="text-[9px] sm:text-[10px] font-black uppercase tracking-wider">Layers</span></button>
             <button onClick={() => setMobileTab('settings')} className={`flex-1 flex flex-col items-center justify-center gap-1.5 transition-all ${mobileTab === 'settings' ? 'text-cyan-400 border-t-2 border-cyan-500' : 'text-slate-500 hover:text-slate-300 border-t-2 border-transparent'}`}><div className="w-5 h-5 sm:w-6 sm:h-6"><PixIcons.Focus /></div><span className="text-[9px] sm:text-[10px] font-black uppercase tracking-wider">Kanvas</span></button>
          </div>
       </div>
