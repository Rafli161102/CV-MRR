...
export const ControlHeader = ({ title, onReset }) => (
  <div className="flex items-center justify-between pb-3 border-b border-[#1f1f1f] mb-5">
     ...
     {onReset && (<button onClick={onReset} className="text-[9px] text-slate-300 hover:text-white bg-[#1a1a1a] border border-[#333] hover:border-red-500/50 px-3 py-1.5 rounded-md transition-colors uppercase tracking-wider shadow-sm">Reset</button>)}
  </div>
);
  ...
  return (
    <div className="flex flex-col sm:flex-row sm:items-center justify-between py-3 border-b border-[#1f1f1f] last:border-0 gap-3 sm:gap-0">
      ...
      <div className="w-full sm:w-2/3 flex items-center gap-3">
        <input type="range" min={min} max={max} step={step} value={value || 0} onChange={(e) => onChange(Number(e.target.value) || 0)} className="w-full h-[5px] bg-[#333] rounded-lg appearance-none cursor-pointer outline-none [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:w-4 [&::-webkit-slider-thumb]:h-4 [&::-webkit-slider-thumb]:bg-cyan-400 [&::-webkit-slider-thumb]:rounded-full hover:[&::-webkit-slider-thumb]:scale-110 transition-all" />
        <div className="bg-[#141414] px-2 py-1.5 rounded-lg border border-[#333] min-w-[70px] flex items-center justify-end shrink-0 cursor-text focus-within:border-cyan-500 transition-colors">
        ...
        <div className="absolute z-50 top-full left-0 right-0 mt-2 bg-[#141414] border border-[#2a2a2a] rounded-xl shadow-2xl max-h-[250px] overflow-y-auto custom-scroll">
          {Object.entries(groups).map(([groupName, options]) => (
            <div key={groupName}>
              ...
              {options.map(opt => (
                <div key={opt.val} onClick={() => { onChange(opt.val); setIsOpen(false); }} className={`px-5 py-3.5 text-[11px] cursor-pointer hover:bg-[#1f1f1f] font-mono ${value === opt.val ? 'bg-cyan-500/10 text-cyan-400 border-l-2 border-cyan-500' : 'text-slate-300'}`}>{opt.name}</div>
              ))}
            </div>
          ))}
        </div>
  ...
  return (
    <div className={`w-full h-full bg-[#0a0a0a] relative flex flex-col overflow-hidden ${isMobileTab ? '' : 'border-t lg:border border-[#1f1f1f] lg:rounded-2xl shadow-xl'}`}>
       <div className="flex justify-between items-center px-4 py-3 border-b border-[#1f1f1f] bg-[#141414] shrink-0 overflow-x-auto custom-scroll">
          ...
          <button onClick={handleCopy} className="flex items-center gap-2 px-4 py-2 rounded-lg bg-cyan-500/10 border border-cyan-500/30 text-cyan-400 hover:bg-cyan-500 hover:text-black transition-all text-[10px] font-bold uppercase shrink-0">
  ...
  return (
    <div className="flex flex-col lg:flex-row w-full h-full animate-fade-in bg-[#050505] lg:bg-transparent">
       ...
       <div className="flex-1 lg:w-[420px] lg:flex-none bg-[#0a0a0a] flex flex-col z-20 overflow-hidden shadow-2xl relative">
         <div className="px-6 py-5 border-b border-[#1f1f1f] bg-[#0a0a0a] shrink-0 z-10 relative">
            <h2 className="text-[14px] font-black text-white uppercase tracking-widest hidden lg:flex items-center gap-2">
         ...
         <div className="flex-1 overflow-y-auto custom-scroll relative bg-[#0a0a0a]">
            <div className={`p-6 lg:p-8 lg:block ${mobileTab === 'design' ? 'block' : 'hidden'}`}>{controls}<div className="h-40 lg:h-12"></div></div>
