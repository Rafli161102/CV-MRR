...
// =========================================================================
// IKON SVG PROFESIONAL (Sangat Minimalis ala Figma / VS Code)
// =========================================================================
const Icons = {
...
export default function CssStudioPage() {
  ...
  // FIX VERCEL: Deklarasi layFlex agar tidak error saat build
  ...
  // =========================================================================
  const generateCSS = () => {
    let css = `/* Generated by MRR CSS Studio */\n\n.element {\n`;
    ...
    return (
      <div className="mb-4">
        ...
        <div className="bg-[#1e1e1e] border border-[#333] rounded-lg overflow-hidden transition-all duration-300 shadow-sm">
          ...
          {/* INLINE PANEL (Mendorong elemen ke bawah secara normal, BUKAN popup absolute) */}
  ...
  return (
    // Figma/VS Code Theme Base: Background sangat gelap (#111111)
    <div className="h-screen w-full flex flex-col font-sans bg-[#1e1e1e] text-[#d4d4d4] overflow-hidden">
      
      {/* HEADER NAV (Tinggi Fix) */}
      <div className="h-14 px-4 sm:px-6 border-b border-[#333333] flex items-center justify-between bg-[#252526] z-50 shrink-0 shadow-sm">
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="px-1.5 py-0.5 bg-cyan-500/20 text-cyan-400 rounded text-[8px] font-bold uppercase tracking-widest hidden sm:block">V8.0 Enterprise</span>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col lg:flex-row overflow-hidden relative">
        ...
        {/* KOLOM 1: TOOLBAR (Kiri - Lebar Fix) */}
        <div className="w-full lg:w-[60px] xl:w-[200px] bg-[#252526] border-b lg:border-b-0 lg:border-r border-[#333333] shrink-0 flex flex-row lg:flex-col gap-1 overflow-x-auto p-2 lg:p-3 [&::-webkit-scrollbar]:hidden z-40">
          ...
          <ToolButton id="filters" title="Filters" icon={<Icons.Filters />} />
          <ToolButton id="transform" title="3D Move" icon={<Icons.Cube3D />} />
          <ToolButton id="text-gradient" title="Txt Gradient" icon={<Icons.TextGrad />} />
          ...
          <div className="flex-1 relative flex items-center justify-center overflow-hidden">
            ...
            <div 
              ...
              style={{
                ...
                ...(activeTab === 'animation' && previewShape !== 'text' ? { background: 'linear-gradient(135deg, #18181b, #252526)', border: '1px solid rgba(6, 182, 212, 0.3)', animation: animType === 'float' ? `float ${animDuration}s ease-in-out infinite` : animType === 'pulse' ? `pulse ${animDuration}s infinite` : `spin ${animDuration}s linear infinite` } : {}),
              }}
            >
        ...
        <div className="w-full lg:w-[280px] shrink-0 bg-[#252526] border-l border-[#333333] flex flex-col z-40 h-[50vh] lg:h-full">
          ...
          <div className="p-4 overflow-y-auto custom-scroll flex-grow bg-[#1e1e1e]">
            ...
            {activeTab === 'typography' && (
              <div className="animate-fade-in space-y-1">
                {previewShape !== 'text' && <div className="p-2 mb-3 bg-[#3f3f46] border border-[#555] rounded text-[9px] text-slate-300 font-medium">Ubah Target ke "TEXT" di panel atas.</div>}
            ...
            {activeTab === 'text-gradient' && (
              <div className="animate-fade-in space-y-1">
                <div className="p-2 mb-3 bg-[#3f3f46] border border-[#555] rounded text-[9px] text-slate-300 font-medium">Ini adalah shortcut cepat untuk Gradient.</div>
                <FigmaSlider label="Angle" min="0" max="360" value={gradAngle} onChange={setGradAngle} />
                <FigmaColorPicker label="Color 1" hexValue={gradColor1} onChange={setGradColor1} />
                <FigmaColorPicker label="Color 2" hexValue={gradColor2} onChange={setGradColor2} />
              </div>
                ...
                <div className="mt-4 pt-4 border-t border-[#333]">
                  ...
                  <div className="flex bg-[#111111] p-1 rounded border border-[#333]">
                    {['flat', 'concave', 'convex'].map(shape => (
                      <button key={shape} onClick={() => setNeoShape(shape)} className={`flex-1 py-1 rounded-sm text-[9px] font-bold uppercase transition-all ${neuShape === shape ? 'bg-[#27272a] text-white' : 'text-slate-500 hover:text-slate-300'}`}>{shape}</button>
                    ))}
                  </div>
                </div>
              </div>
            ...
            {activeTab === 'filters' && (
              <div className="animate-fade-in space-y-1">
                <div className="p-2 mb-3 bg-[#3f3f46] border border-[#555] rounded text-[9px] text-slate-300 font-medium">Filter mengubah warna dasar gambar secara langsung.</div>
                ...
                <div className="mb-4">
                  ...
                  <div className="flex flex-col gap-1.5">
                    {[{ id: 'float', name: 'Floating' }, { id: 'pulse', name: 'Pulse Glow' }, { id: 'spin', name: 'Spin Rotate' }].map(anim => (
                      <button key={anim.id} onClick={() => setAnimType(anim.id)} className={`px-3 py-2 rounded text-[10px] font-bold text-left transition-all ${animType === anim.id ? 'bg-cyan-500/20 text-cyan-400 border border-cyan-500/50' : 'bg-[#111111] text-slate-400 border border-[#333] hover:bg-[#2c2c2e]'}`}>
                        {anim.name}
                      </button>
                    ))}
                  </div>
