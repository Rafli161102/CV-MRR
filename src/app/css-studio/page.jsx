...
export default function CssStudioPage() {
  ...
  return (
    // FIX BUG 2: pt-[75px] sm:pt-[90px] mendorong UI editor agar tidak tertimpa Navbar Global di HP
    <div className="h-screen w-full flex flex-col font-sans bg-[#111111] text-[#d4d4d4] overflow-hidden pt-[75px] sm:pt-[85px] lg:pt-[90px]">
      ...
      <div className="flex-1 flex flex-col lg:flex-row overflow-hidden relative">
        ...
        {/* FIX BUG 3: Toolbar di HP sekarang jadi flex baris yang gampang di klik (flat layout) */}
        <div className="w-full lg:w-[60px] xl:w-[220px] bg-[#18181b] border-b lg:border-b-0 lg:border-r border-[#252526] shrink-0 flex flex-row lg:flex-col lg:gap-1 overflow-x-auto lg:p-3 [&::-webkit-scrollbar]:hidden z-50">
           {PLUGINS.map(p => (
              <button 
                key={p.id} 
                onClick={() => setActiveId(p.id)} 
                className={`flex items-center gap-3 w-auto xl:w-full p-3 lg:px-3 lg:py-2.5 rounded-none lg:rounded-lg transition-all duration-200 border-b-[3px] lg:border-b-0 lg:border-l-[3px] shrink-0 relative z-50 pointer-events-auto ${activeId === p.id ? 'bg-[#2c2c2e] border-cyan-500 text-white shadow-sm' : 'bg-transparent border-transparent text-slate-400 hover:bg-[#2c2c2e]/50 hover:text-slate-200'}`}
              >
