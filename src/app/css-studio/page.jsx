...
// =========================================================================
// 📦 VIRTUAL FILE: shared.jsx (Berisi Ikon, Core Functions, dan UI)
// =========================================================================
...
// 2. FUNGSI KEAMANAN WARNA (Anti Crash & Anti Bug)
const safeHex = (hex) => (typeof hex === 'string' && hex.startsWith('#') && (hex.length === 4 || hex.length === 7)) ? hex : '#000000';
...
// Inline Color Picker Anti-Popup Murni
      ...
      <div className="bg-[#111111] border border-[#252526] rounded-lg overflow-hidden transition-all duration-300 shadow-sm">
        <div className="flex items-center justify-between p-2 cursor-pointer hover:bg-[#1a1a1c] transition-colors" onClick={() => setIsOpen(!isOpen)}>
          <div className="flex items-center gap-2.5">
            ...
            <span className="text-[9px] font-mono text-slate-300 uppercase">{safeColor}</span>
          </div>
...
// Workspace Master Layout (Responsif Total)
  ...
  return (
    <div className="flex flex-col lg:flex-row w-full h-full animate-fade-in bg-[#0a0a0b] lg:bg-transparent">
       ...
       <div className="w-full lg:w-[320px] bg-[#18181b] flex flex-col shrink-0 z-20">
         ...
         {/* Properties Scroll */}
         <div className="p-4 overflow-y-auto custom-scroll flex-1 min-h-[30vh]">
            {controls}
         </div>
         {/* CODE OUTPUT (Mobile Only - diletakkan di paling bawah setelah Properties) */}
         <div className="lg:hidden h-[200px] border-t border-[#252526] shrink-0">
            <CodeOutput code={cssOutput} />
         </div>
       </div>
    </div>
...
// =========================================================================
// 📦 VIRTUAL FILE: plugins.jsx (Kumpulan Modul Terisolasi Penuh)
// =========================================================================
...
const PluginTextGradient = () => {
  ...
  const preview = (
     <div className="w-full text-center px-4 overflow-hidden">
       {/* Bug Fix: Mencegah box gradient pada HP dengan display: inline-block */}
       <h2 className="tracking-tighter" style={{ display: 'inline-block', fontSize: `${size}px`, fontWeight: weight, background: `linear-gradient(${angle}deg, ${color1}, ${color2})`, WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text', color: 'transparent', lineHeight: 1.1 }}>{text}</h2>
     </div>
  );
  const controls = (
     <>
...
const PluginGlassmorphism = () => {
   ...
   const preview = (
     <div style={{ width: shape === 'circle' ? '220px' : shape === 'pill' ? '320px' : '220px', height: shape === 'circle' ? '220px' : shape === 'pill' ? '100px' : '220px', borderRadius: shape === 'circle' ? '50%' : shape === 'pill' ? '50px' : '24px', background: hexToRgba(color, opacity), backdropFilter: `blur(${blur}px)`, WebkitBackdropFilter: `blur(${blur}px)`, border: `1px solid ${hexToRgba('#ffffff', outline)}`, boxShadow: '0 8px 32px 0 rgba(0, 0, 0, 0.2)' }} className="flex items-center justify-center transition-all duration-500 overflow-hidden">
...
const PluginNeumorphism = () => {
   ...
   const preview = (
     <div style={{ width: shape === 'circle' ? '200px' : shape === 'pill' ? '300px' : '200px', height: shape === 'circle' ? '200px' : shape === 'pill' ? '100px' : '200px', borderRadius: shape === 'circle' ? '50%' : shape === 'pill' ? '50px' : '24px', background: bgGradient, boxShadow: `${darkShadow}, ${lightShadow}` }} className="flex items-center justify-center transition-all duration-500 overflow-hidden">
   ...
   const controls = (
     <>
        ...
        <FigmaSelect label="Target Shape" options={['box', 'circle', 'pill']} value={shape} onChange={setShape} />
        <FigmaSelect label="Lighting Type" options={['flat', 'concave', 'convex']} value={neuType} onChange={setNeuType} />
        <div className="my-4 border-t border-[#252526] pt-4"></div>
...
const PluginShadow = () => {
   ...
   const preview = (
     <div style={{ width: shape === 'circle' ? '180px' : shape === 'pill' ? '280px' : '180px', height: shape === 'circle' ? '180px' : shape === 'pill' ? '90px' : '180px', borderRadius: shape === 'circle' ? '50%' : shape === 'pill' ? '50px' : '24px', background: '#ffffff', boxShadow: shadows.join(', ') }} className="flex items-center justify-center transition-all duration-500 overflow-hidden">
...
const PluginGlow = () => {
   ...
   const preview = (
     <div style={{ width: shape === 'circle' ? '180px' : shape === 'pill' ? '280px' : '180px', height: shape === 'circle' ? '180px' : shape === 'pill' ? '90px' : '180px', borderRadius: shape === 'circle' ? '50%' : shape === 'pill' ? '50px' : '24px', background: '#111111', border: `1px solid ${hexToRgba(color, 0.5)}`, boxShadow: `0 0 ${blur}px ${spread}px ${hexToRgba(color, opacity)}, inset 0 0 15px ${hexToRgba(color, opacity * 0.5)}` }} className="flex items-center justify-center transition-all duration-500 overflow-hidden">
...
const PLUGINS = [
  ...
  { id: 'transform', title: '3D Transform', icon: <Icons.Cube3D />, component: PluginTransform, cat: 'Advanced' },
...
export default function CssStudioPage() {
  ...
  return (
    // FIX VERCEL BUG 2: Padding Top (pt-[75px]) memastikan Header Studio tidak tertimpa Navbar Global
    <div className="h-screen w-full flex flex-col font-sans bg-[#111111] text-[#d4d4d4] overflow-hidden pt-[75px] sm:pt-[85px] lg:pt-[90px]">
      
      {/* HEADER NAV (Internal Studio) */}
      <div className="h-12 px-4 sm:px-6 border-b border-[#252526] flex items-center justify-between bg-[#18181b] z-30 shrink-0 shadow-sm relative">
        <div className="flex items-center gap-4">
          <Link href="/toolkit" className="text-slate-400 hover:text-white transition-colors flex items-center justify-center p-1.5 rounded-md hover:bg-white/10"><Icons.ArrowLeft /></Link>
          <div className="flex items-center gap-2">
            <span className="font-bold text-white tracking-tight text-sm">CSS Visual <span className="text-cyan-500">Studio</span></span>
            <span className="px-1.5 py-0.5 bg-cyan-500/20 text-cyan-400 rounded text-[8px] font-bold uppercase tracking-widest hidden sm:block">V11 Enterprise</span>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col lg:flex-row overflow-hidden relative">
        ...
        {/* FIX MOBILE: Container ini dibuat overflow-y-auto agar Canvas yang Sticky di dalamnya bisa bekerja */}
        <div className="flex-1 overflow-y-auto lg:overflow-hidden bg-[#111111] p-4 lg:p-6 relative">
