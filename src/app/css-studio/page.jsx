...
const FigmaColorPicker = ({ label, hexValue, onChange }) => {
  ...
  return (
    <div className="mb-3">
      <div className="flex items-center justify-between py-1.5 cursor-pointer group" onClick={() => setIsOpen(!isOpen)}>
         <label className="text-[10px] font-medium text-slate-400 group-hover:text-slate-200 transition-colors cursor-pointer">{label}</label>
         <div className="flex items-center gap-2 bg-[#111111] border border-[#333] rounded px-1.5 py-0.5 group-hover:border-[#555] transition-colors">
            <div className="w-3.5 h-3.5 rounded shadow-inner border border-white/10" style={{backgroundColor: hexValue}}></div>
            <span className="text-[9px] font-mono text-slate-300 uppercase">{hexValue}</span>
         </div>
      </div>
      
      <div className={`overflow-hidden transition-all duration-300 ease-in-out ${isOpen ? 'max-h-[250px] opacity-100 bg-[#111111] p-3 rounded-lg border border-[#333] mt-1' : 'max-h-0 opacity-0'}`}>
...
const FigmaSelect = ({ label, options, value, onChange }) => (
  <div className="mb-3 mt-1">
     <label className="text-[10px] font-medium text-slate-400 block mb-1.5">{label}</label>
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-4 lg:gap-6 w-full h-full pb-10 lg:pb-0 animate-fade-in">
       ...
       <div className="order-1 lg:order-2 flex-1 flex flex-col gap-4 overflow-visible lg:overflow-hidden h-auto lg:h-[calc(100vh-100px)]">
         <div className="h-[350px] lg:h-auto lg:flex-1 rounded-2xl border border-[#252526] relative flex items-center justify-center overflow-hidden bg-[#0a0a0b] shadow-inner sticky top-4 lg:static z-10 transition-colors duration-500" style={{backgroundColor: bgHex || '#0a0a0b'}}>
...
const PluginBorder = () => {
  ...
  const preview = (
    <div style={{ width: shape === 'circle' ? '200px' : shape === 'pill' ? '300px' : '200px', height: shape === 'circle' ? '200px' : shape === 'pill' ? '100px' : '200px', border: `${width}px ${style} ${color}`, borderRadius: shape === 'circle' ? '50%' : shape === 'pill' ? '50px' : `${radius}px` }} className="flex items-center justify-center transition-all duration-500">
...
const PluginTextGradient = () => {
  ...
  const preview = (
     <div className="w-full text-center px-4">
       <h2 className="tracking-tighter transition-all duration-300" style={{ fontSize: `${size}px`, fontWeight: weight, background: `linear-gradient(${angle}deg, ${color1}, ${color2})`, WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text', color: 'transparent', lineHeight: 1.1 }}>{text}</h2>
     </div>
  );
  const controls = (
     <>
