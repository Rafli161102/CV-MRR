...
export default function RateTab() {
  ...
  const [hoursPerItem, setHoursPerItem] = useState(""); 
  ...
  else if (calcMode === 'gig') totalHours = numWorkHours + numPrepHours; 
  else if (calcMode === 'batch') totalHours = numItemCount * numHoursPerItem; 
  else if (calcMode === 'shift') totalHours = numShiftCount * numHoursPerShift; 
  ...
  const pricePerItem = numItemCount > 0 ? finalTotalFee / numItemCount : 0; 
  const pricePerShift = numShiftCount > 0 ? finalTotalFee / numShiftCount : 0; 
    ...
    <div className="print:hidden grid grid-cols-1 lg:grid-cols-12 gap-8 anim-fade-in-up">
        ...
        <div className="bg-gradient-to-b from-[#0a0a0a] to-[#050505] border border-white/5 rounded-[2rem] p-6 sm:p-10 shadow-2xl relative overflow-hidden">
          ...
          <div className="space-y-8">
            <CleanSelect 
              ...
              options={[{ value: "1", label: "Entry Level / Pemula (1x)" }, { value: "1.5", label: "Mid-Level / Berpengalaman (1.5x)" }, { value: "2.5", label: "Senior / Pro / Expert (2.5x)" }]}
            />
            <CleanInput 
              label={
                calcMode === 'monthly' ? "Target Penghasilan Bersih (Per Bulan)" : 
                calcMode === 'project' ? "Target Upah Bersih (Satu Projek Ini)" :
                calcMode === 'batch' ? "Target Upah Bersih (Untuk Seluruh Konten)" :
                calcMode === 'shift' ? "Target Upah Bersih (Seluruh Hari/Shift)" :
            ...
            <CleanInput 
              label={
                ...
                calcMode === 'project' ? "Biaya Tambahan Projek (Aset, Server, Lisensi)" :
            ...
            <div className={`grid ${calcMode === 'project' || calcMode === 'task' ? 'grid-cols-1' : 'grid-cols-1 sm:grid-cols-2'} gap-6`}>
              ...
              {calcMode === 'batch' && <CleanInput label="Jumlah Output (Item/Video)" name="itemCount" type="number" value={itemCount} onChange={e => setItemCount(e.target.value)} placeholder="10" />}
              {calcMode === 'shift' && <CleanInput label="Jumlah Hari Turun / Shift" name="shiftCount" type="number" value={shiftCount} onChange={e => setShiftCount(e.target.value)} placeholder="3" />}
      ...
      <div className="lg:col-span-5">
        <div className="sticky top-10 bg-gradient-to-b from-cyan-950/20 to-[#0a0a0a] border border-cyan-500/20 rounded-[2rem] p-6 sm:p-10 shadow-[0_20px_40px_rgba(6,182,212,0.1)]">
          <h3 className="text-xs font-black text-cyan-400 uppercase tracking-[0.2em] mb-8 flex items-center gap-2"><span className="w-2 h-2 rounded-full bg-cyan-400 animate-pulse"></span> Hasil Kalkulasi Final</h3>
          <div className="space-y-8">
            {calcMode === 'monthly' && (
               <>
                  <div>
            ...
            <div className="bg-[#030712]/80 backdrop-blur-md rounded-2xl p-6 border border-white/5 relative overflow-hidden group mt-6">
              <div className={`absolute left-0 top-0 w-1.5 h-full ${calcMode === 'gig' || calcMode === 'task' ? 'bg-gradient-to-b from-indigo-400 to-purple-500' : calcMode === 'batch' || calcMode === 'shift' ? 'bg-gradient-to-b from-blue-400 to-cyan-500' : 'bg-gradient-to-b from-emerald-400 to-cyan-500'}`}></div>
              <p className="text-xs text-slate-300 leading-relaxed pl-3 font-medium">💡 <strong className="text-white">Insights:</strong> Jadikan hasil ini harga dasar (Floor Price). Naikkan 20-50% jika klien adalah perusahaan besar berskala Corporate.</p>
            </div>
          </div>
        </div>
      </div>
