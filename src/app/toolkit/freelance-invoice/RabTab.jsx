...
// --- KAMUS TRANSLATE (AI MULTI-LANGUAGE UNTUK RAB) ---
const dict = {
  id: {
    title: "PENAWARAN", ref: "Referensi Proyek", date: "Tanggal Terbit", validUntil: "Masa Berlaku",
    from: "Informasi Penawar", to: "Ditawarkan Kepada",
    desc: "Deskripsi Layanan / Item", qty: "Qty", price: "Harga Satuan", total: "Jumlah",
    subtotal: "Subtotal", discount: "Diskon", tax: "Pajak", grandTotal: "Total Estimasi",
    payment: "Opsi Pembayaran (Bila Disetujui)", bank: "Bank/Platform", accNum: "No. Rekening/Akun", accName: "Atas Nama", swift: "SWIFT / Routing",
    notes: "Catatan & Syarat Ketentuan", signature: "Hormat Kami,", companyPlaceholder: "NAMA PERUSAHAAN"
  },
  en: {
    title: "QUOTATION", ref: "Project Reference", date: "Date of Issue", validUntil: "Valid Until",
    from: "From (Sender)", to: "Quoted To (Client)",
    desc: "Service Description", qty: "Qty", price: "Unit Price", total: "Amount",
    subtotal: "Subtotal", discount: "Discount", tax: "Tax", grandTotal: "Estimated Total",
    payment: "Payment Options", bank: "Bank/Platform", accNum: "Account Number", accName: "Account Name", swift: "SWIFT / Routing",
    notes: "Notes & Terms", signature: "Sincerely,", companyPlaceholder: "COMPANY NAME"
  }
};
...
const SignatureBlock = ({ quotationData, stamp, themeColor, t }) => (
  <div className="relative inline-flex flex-col items-center justify-end pt-2 min-w-[200px] page-break-inside-avoid">
    ...
    <div className="relative h-20 flex items-center justify-center w-full">
      <span className="text-[40px] opacity-80 z-10 whitespace-nowrap" style={{ color: themeColor, fontFamily: "'Brush Script MT', 'Lucida Handwriting', cursive" }}>
        {quotationData.signatureName || quotationData.myName || "Signature"}
      </span>
      {stamp && (
         <img 
           src={stamp} 
    ...
    <p className="text-[12px] font-black text-slate-800 z-10 relative text-center w-full">{quotationData.myName || "Nama Pengirim"}</p>
    {quotationData.myRole && <p className="text-[9px] text-slate-500 font-bold uppercase tracking-widest mt-0.5 z-10 relative text-center w-full">{quotationData.myRole}</p>}
  </div>
);

// 1. Template Modern
const TemplateModern = ({ quotationData, items, logo, stamp, themeColor, currency, subtotal, taxAmount, discountAmount, total, t }) => (
    ...
    <div className="p-10 relative z-10 flex-grow flex flex-col">
        ...
        <div style={{ flex: 1.618 }}>
          {logo ? (
            ...
            <h2 className="text-2xl font-black tracking-tight mb-2 transition-colors duration-300 uppercase print:!text-current" style={{ color: themeColor }}>
              {quotationData.myName || t.companyPlaceholder}
            </h2>
          )}
          {quotationData.projectName && (
            <div className="mt-1 inline-block bg-slate-50 border border-slate-200 px-3 py-1.5 rounded-xl">
              ...
              <p className="text-[12px] font-bold text-slate-800">{quotationData.projectName}</p>
            </div>
          )}
        </div>
        <div className="text-right" style={{ flex: 1 }}>
          <h1 className="text-3xl font-black uppercase tracking-widest mb-1 transition-colors duration-300 print:!text-current" style={{ color: themeColor }}>{t.title}</h1>
          <p className="text-slate-500 font-bold tracking-wider mb-2 text-sm">#{quotationData.quotationNo || "RAB-XXXX"}</p>
          <div className="flex flex-col items-end gap-1 text-[9px] mt-2 bg-slate-50 p-2.5 rounded-xl border border-slate-100">
            <div className="flex justify-between w-full">
              ...
              <p className="text-slate-800 font-semibold">{quotationData.date || "-"}</p>
            </div>
            <div className="w-full h-px bg-slate-200 my-0.5"></div>
            <div className="flex justify-between w-full">
              <p className="text-slate-400 font-bold uppercase tracking-widest">{t.validUntil}</p>
              <p className="text-slate-800 font-semibold">{quotationData.validUntil || "-"}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-row gap-6 mb-6 bg-slate-50/70 p-4 rounded-2xl border border-slate-100/80 shrink-0">
        <div className="border-r border-slate-200/60 pr-5" style={{ flex: 1 }}>
          ...
          <p className="text-[12px] font-bold text-slate-800 mb-1 leading-tight">{quotationData.myName || "Nama Pengirim"}</p>
          {quotationData.myRole && <p className="text-[10px] font-medium text-slate-500 mb-1">{quotationData.myRole}</p>}
          {quotationData.myEmail && <p className="text-[10px] text-slate-600 mb-0.5">{quotationData.myEmail}</p>}
          {quotationData.myPhone && <p className="text-[10px] text-slate-600 mb-1.5">{quotationData.myPhone}</p>}
          {quotationData.myTaxId && <p className="text-[9px] font-bold text-slate-500 bg-slate-200/50 px-2 py-0.5 rounded-md inline-block">Tax ID: {quotationData.myTaxId}</p>}
        </div>
        <div className="pl-1" style={{ flex: 1.618 }}>
          ...
          <p className="text-[12px] font-bold text-slate-800 mb-1 leading-tight">{quotationData.clientName || "Nama Klien / Perusahaan"}</p>
          {quotationData.clientAddress && <p className="text-[11px] text-slate-600 whitespace-pre-wrap leading-relaxed mb-1.5">{quotationData.clientAddress}</p>}
          {quotationData.clientTaxId && <p className="text-[9px] font-bold text-slate-500 bg-slate-200/50 px-2 py-0.5 rounded-md inline-block">Tax ID: {quotationData.clientTaxId}</p>}
        </div>
      </div>
            ...
            {items.map((item, index) => (
              <tr key={item.id} className="hover:bg-slate-50/50 transition-colors break-inside-avoid">
                <td className="py-2.5 px-4">
                  <p className="text-[11px] text-slate-800 font-bold leading-tight">{item.description || `Item Estimasi #${index + 1}`}</p>
      ...
      <div className="flex flex-row items-start gap-6 mt-auto pt-2 break-inside-avoid pb-6">
        <div className="space-y-4" style={{ flex: 1.618 }}>
          {(quotationData.bankName || quotationData.accNumber || quotationData.accName || quotationData.bankCode) && (
            <div>
              ...
              <div className="bg-slate-50 rounded-xl p-3 border border-slate-100 shadow-sm relative overflow-hidden">
                ...
                {quotationData.bankName && <p className="text-[10px] font-bold text-slate-800 mb-0.5">{quotationData.bankName}</p>}
                {quotationData.accNumber && <p className="text-[13px] font-mono font-black text-slate-700 mb-1 tracking-widest">{quotationData.accNumber}</p>}
                {quotationData.accName && <p className="text-[9px] text-slate-500 font-bold uppercase tracking-wider mb-1">{t.accName}: {quotationData.accName}</p>}
                {quotationData.bankCode && <p className="text-[8px] text-slate-500 font-bold uppercase tracking-wider mt-1.5 pt-1.5 border-t border-slate-200">{t.swift}: <span className="font-mono text-slate-700">{quotationData.bankCode}</span></p>}
              </div>
            </div>
          )}
          {quotationData.notes && (
            <div>
              ...
              <p className="text-[9px] text-slate-600 font-medium whitespace-pre-wrap leading-relaxed bg-slate-50/50 p-3 rounded-xl border border-slate-100/80">{quotationData.notes}</p>
            </div>
          )}
        </div>
        ...
        <div className="flex flex-col gap-4" style={{ flex: 1 }}>
          <div className="w-full space-y-1.5 bg-slate-50 p-3 rounded-2xl border border-slate-100 shadow-sm">
            ...
            {Number(quotationData.discount) > 0 && (
              <div className="flex justify-between items-center text-[10px]">
                <span className="text-slate-500 font-bold">{t.discount} {quotationData.discountType === 'percent' ? `(${quotationData.discount}%)` : ''}</span>
                <span className="text-red-500 font-bold">-{formatCurrency(discountAmount, currency)}</span>
              </div>
            )}
            {Number(quotationData.taxRate) > 0 && (
              <div className="flex justify-between items-center text-[10px]">
                <span className="text-slate-500 font-bold">{t.tax} ({quotationData.taxRate}%)</span>
          ...
          <div className="flex justify-end w-full">
            <SignatureBlock quotationData={quotationData} stamp={stamp} themeColor={themeColor} t={t} />
          </div>
        </div>
      </div>
    </div>
...
const TemplateMinimal = ({ quotationData, items, logo, stamp, themeColor, currency, subtotal, taxAmount, discountAmount, total, t }) => (
  <div className="relative bg-white text-slate-900 overflow-hidden w-full h-full p-10 font-sans flex flex-col shadow-[0_10px_40px_rgba(0,0,0,0.15)] print:shadow-none">
    ...
    <div className="flex flex-row justify-between items-end border-b-[2px] pb-4 mb-6 transition-colors duration-300 shrink-0 print:!border-current" style={{ borderColor: themeColor }}>
      <div style={{ flex: 1.618 }}>
        <h1 className="text-4xl font-light tracking-widest mb-1.5 text-slate-900">{t.title}</h1>
        <p className="text-slate-500 font-bold tracking-widest text-sm">#{quotationData.quotationNo || "RAB-XXXX"}</p>
      </div>
    ...
    <div className="flex flex-row gap-6 mb-6 shrink-0">
      <div className="space-y-3" style={{ flex: 1.618 }}>
        <div>
          ...
          <p className="text-[13px] font-bold text-slate-800 leading-tight mb-1">{quotationData.clientName || "Nama Klien / Perusahaan"}</p>
          {quotationData.clientAddress && <p className="text-[11px] text-slate-600 whitespace-pre-wrap leading-relaxed">{quotationData.clientAddress}</p>}
          {quotationData.clientTaxId && <p className="text-[9px] text-slate-500 mt-1.5 font-medium">Tax ID: {quotationData.clientTaxId}</p>}
        </div>
        {quotationData.projectName && (
          <div>
            <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mb-1">{t.ref}:</p>
            <p className="text-[11px] font-bold text-slate-800">{quotationData.projectName}</p>
          </div>
        )}
      </div>
      ...
      <div className="space-y-4 text-right" style={{ flex: 1 }}>
        <div>
          ...
          <p className="text-[12px] font-bold text-slate-800 mb-0.5">{quotationData.myName || "Nama Anda"}</p>
          {quotationData.myEmail && <p className="text-[10px] text-slate-600">{quotationData.myEmail}</p>}
          {quotationData.myPhone && <p className="text-[10px] text-slate-600">{quotationData.myPhone}</p>}
          {quotationData.myTaxId && <p className="text-[9px] text-slate-500 mt-1 font-medium">Tax ID: {quotationData.myTaxId}</p>}
        </div>
        <div className="grid grid-cols-2 gap-2 text-left bg-slate-50 p-2.5 rounded-xl">
          <div>
            ...
            <p className="text-[10px] font-bold text-slate-800">{quotationData.date || "-"}</p>
          </div>
          <div>
            <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mb-0.5">{t.validUntil}:</p>
            <p className="text-[10px] font-bold text-slate-800">{quotationData.validUntil || "-"}</p>
          </div>
        </div>
      </div>
    </div>
        ...
        {items.map((item, index) => (
          <tr key={item.id} className="break-inside-avoid">
            <td className="py-2.5 px-2">
              <p className="text-[11px] text-slate-800 font-bold">{item.description || `Item Estimasi #${index + 1}`}</p>
    ...
    <div className="flex flex-row items-start gap-6 mt-auto pt-4 break-inside-avoid pb-4">
        ...
        {(quotationData.bankName || quotationData.accNumber || quotationData.accName || quotationData.bankCode) && (
          <div>
            ...
            <div className="p-3 border border-slate-200 bg-slate-50 rounded-xl">
              {quotationData.bankName && <p className="text-[11px] font-bold text-slate-800 mb-0.5">{quotationData.bankName}</p>}
              {quotationData.accNumber && <p className="text-[13px] font-mono font-black text-slate-800 tracking-wider mb-1">{quotationData.accNumber}</p>}
              {quotationData.accName && <p className="text-[9px] text-slate-500 font-bold uppercase tracking-widest mt-0.5">{t.accName}: {quotationData.accName}</p>}
              {quotationData.bankCode && <p className="text-[9px] text-slate-500 font-bold uppercase tracking-widest mt-1.5 pt-1.5 border-t border-slate-200">{t.swift}: <span className="font-mono text-slate-800">{quotationData.bankCode}</span></p>}
            </div>
          </div>
        )}
        {quotationData.notes && (
          <div>
            ...
            <p className="text-[10px] text-slate-600 whitespace-pre-wrap leading-relaxed">{quotationData.notes}</p>
          </div>
        )}
      </div>
      ...
      <div className="flex flex-col items-end" style={{ flex: 1 }}>
        <div className="w-full space-y-2 border-b-[2px] border-slate-800 pb-3 mb-3">
          ...
          {Number(quotationData.discount) > 0 && (
            <div className="flex justify-between text-[10px]">
              <span className="text-slate-600 font-bold">{t.discount} {quotationData.discountType === 'percent' ? `(${quotationData.discount}%)` : ''}</span>
              <span className="font-black text-red-600">-{formatCurrency(discountAmount, currency)}</span>
            </div>
          )}
          {Number(quotationData.taxRate) > 0 && (
            <div className="flex justify-between text-[10px]">
              <span className="text-slate-600 font-bold">{t.tax} ({quotationData.taxRate}%)</span>
...
// 3. Template Klasik
const TemplateClassic = ({ quotationData, items, logo, stamp, themeColor, currency, subtotal, taxAmount, discountAmount, total, t }) => (
  <div className="relative bg-white text-slate-900 overflow-hidden w-full h-full p-10 shadow-[0_10px_40px_rgba(0,0,0,0.15)] print:shadow-none flex flex-col" style={{ fontFamily: "'Times New Roman', Times, serif" }}>
    <div className="text-center mb-6 border-b-2 border-slate-800 pb-4 shrink-0">
      ...
      <p className="text-[11px] text-slate-700 leading-relaxed max-w-xl mx-auto">{quotationData.clientAddress || "Gedung Pusat Bisnis Lt. 3, Jl. Protokol No. 1, Jakarta Pusat, Indonesia"}</p>
      <p className="text-[11px] text-slate-700 mt-0.5">{quotationData.myEmail} &nbsp;&bull;&nbsp; {quotationData.myPhone}</p>
      {quotationData.myTaxId && <p className="text-[10px] text-slate-600 mt-1">Tax ID: {quotationData.myTaxId}</p>}
    </div>

    <div className="flex flex-row justify-between items-end mb-6 gap-4 shrink-0">
      <h1 className="text-3xl font-bold uppercase tracking-[0.2em] text-slate-900">{t.title}</h1>
      <div className="text-right border border-slate-800 p-2.5 bg-slate-50 min-w-[180px]">
        ...
        <p className="text-base font-bold text-slate-900">{quotationData.quotationNo || "RAB-XXXX-001"}</p>
      </div>
    </div>
    ...
    <div className="flex flex-row gap-6 mb-6 shrink-0">
      <div className="border border-slate-800 p-4 relative" style={{ flex: 1.618 }}>
        ...
        <p className="text-[13px] font-bold text-slate-900 mb-1 mt-1">{quotationData.clientName || "Nama Klien / Instansi"}</p>
        {quotationData.clientAddress && <p className="text-[11px] text-slate-700 whitespace-pre-wrap leading-relaxed">{quotationData.clientAddress}</p>}
        {quotationData.clientTaxId && <p className="text-[9px] text-slate-600 mt-1.5">Tax ID: {quotationData.clientTaxId}</p>}
      </div>
      <div className="flex flex-col gap-3" style={{ flex: 1 }}>
        <div className="border border-slate-800 p-3 relative flex-1">
           <h3 className="absolute -top-2.5 left-3 bg-white px-1.5 text-[9px] font-bold text-slate-800 uppercase tracking-widest">Detail Penawaran</h3>
           <div className="mt-1 flex justify-between">
             <p className="text-[9px] font-bold text-slate-600">{t.date}:</p>
             <p className="text-[10px] font-bold text-slate-900">{quotationData.date || "-"}</p>
           </div>
           <div className="mt-1 flex justify-between">
             <p className="text-[9px] font-bold text-slate-600">{t.validUntil}:</p>
             <p className="text-[10px] font-bold text-slate-900">{quotationData.validUntil || "-"}</p>
           </div>
        </div>
        {quotationData.projectName && (
          <div className="border border-slate-800 p-3 relative flex-1 bg-slate-50">
            ...
            <p className="text-[11px] font-bold text-slate-900 mt-1.5 leading-tight">{quotationData.projectName}</p>
          </div>
        )}
      </div>
    </div>
      ...
      <div className="w-[50%]">
        <table className="w-full border-collapse border border-slate-800">
          <tbody>
            ...
            {Number(quotationData.discount) > 0 && (
              <tr>
                <td className="py-1.5 px-3 border border-slate-800 text-[10px] text-slate-800 font-bold bg-slate-50">{t.discount} {quotationData.discountType === 'percent' ? `(${quotationData.discount}%)` : ''}</td>
            ...
            {Number(quotationData.taxRate) > 0 && (
              <tr>
                <td className="py-1.5 px-3 border border-slate-800 text-[10px] text-slate-800 font-bold bg-slate-50">{t.tax} ({quotationData.taxRate}%)</td>
        ...
        {(quotationData.bankName || quotationData.accNumber || quotationData.accName || quotationData.bankCode) && (
          <div className="mb-4">
            ...
            <div className="text-[10px] text-slate-800 leading-relaxed bg-slate-50 p-2.5 border border-slate-200">
              {quotationData.bankName && <p><strong>{t.bank}:</strong> {quotationData.bankName}</p>}
              {quotationData.accNumber && <p><strong>{t.accNum}:</strong> <span className="font-mono">{quotationData.accNumber}</span></p>}
              {quotationData.accName && <p><strong>{t.accName}:</strong> {quotationData.accName}</p>}
              {quotationData.bankCode && <p className="mt-1 pt-1 border-t border-slate-300"><strong>{t.swift}:</strong> <span className="font-mono">{quotationData.bankCode}</span></p>}
            </div>
          </div>
        )}
        {quotationData.notes && (
          <div>
            ...
            <p className="text-[9px] text-slate-800 whitespace-pre-wrap leading-relaxed">{quotationData.notes}</p>
          </div>
        )}
      </div>
...
export default function RabTab() {
  const [quotationData, setQuotationData] = useState({
    quotationNo: "", date: "", validUntil: "", projectName: "",
  ...
  useEffect(() => {
    ...
    defaultValidUntil.setDate(defaultValidUntil.getDate() + 30); // Default RAB berlaku 30 hari
  ...
  const handleQuotationChange = (e) => setQuotationData({ ...quotationData, [e.target.name]: e.target.value });
  ...
  const setValidityDays = (days) => {
    if (!quotationData.date) return;
    const date = new Date(quotationData.date);
    date.setDate(date.getDate() + days);
    setQuotationData({ ...quotationData, validUntil: date.toISOString().split('T')[0] });
  };
  ...
  const numDiscount = parseFloat(quotationData.discount) || 0;
  ...
  const numTaxRate = parseFloat(quotationData.taxRate) || 0;
  ...
  return (
    <div className="w-full relative pb-20 anim-fade-in-up print-reset-layout print:bg-white print:pb-0">
      ...
      <style dangerouslySetInnerHTML={{__html: `
        ...
        /* ISOLASI PRINT MUTLAK WYSIWYG A4 - FIX BACKGROUND & PARENT VISIBILITY */
        @media print {
          @page { size: A4; margin: 0; }
          html, body, main, section, div#__next, div#root { 
          ...
          * {
          ...
          /* Sembunyikan elemen secara aman HANYA yang memiliki class .no-print */
          .no-print { display: none !important; }
          
          /* Reset Wrapper Kertas untuk mematikan skala dari React dan memulihkan arus dokumen */
          .print-reset-layout {
      ...
      <div className="no-print flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 mb-10 bg-[#0a0a0a] border border-white/5 rounded-[2rem] p-6 sm:p-8 shadow-xl relative overflow-hidden">
        ...
        <div className="relative z-10">
          <h2 className="text-2xl font-black text-white tracking-tight flex items-center gap-3">
            ...
            Penawaran <span className="text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-blue-500">(RAB)</span>
          </h2>
          <p className="text-sm text-slate-400 mt-2 max-w-xl">Buat proposal penawaran harga untuk prospek klien secara instan dengan tata letak padat, WYSIWYG A4 presisi tinggi, dan multi-bahasa terintegrasi.</p>
        </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-8 lg:gap-10 print-reset-layout">
        ...
        <div className="xl:col-span-5 space-y-6 no-print">
          ...
          <div className="bg-[#030712]/80 backdrop-blur-md rounded-2xl p-6 border border-white/5 relative shadow-xl">
             ...
             <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
              <CleanInput 
                ...
                placeholder="Misal: RAB/2026/04/001" 
              />
              <CleanInput 
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <CleanInput label="Tanggal Terbit" type="date" name="date" value={quotationData.date} onChange={handleQuotationChange} />
              <div>
                <CleanInput label="Masa Berlaku" type="date" name="validUntil" value={quotationData.validUntil} onChange={handleQuotationChange} />
                <div className="flex gap-2 mt-2">
                   <button onClick={() => setValidityDays(14)} className="flex-1 text-[9px] font-bold bg-[#030712] border border-white/10 hover:bg-white/10 text-slate-400 px-2 py-1.5 rounded-lg transition-colors shadow-sm">+14 Hari</button>
                   <button onClick={() => setValidityDays(30)} className="flex-1 text-[9px] font-bold bg-[#030712] border border-white/10 hover:bg-white/10 text-slate-400 px-2 py-1.5 rounded-lg transition-colors shadow-sm">+30 Hari</button>
                   <button onClick={() => setValidityDays(60)} className="flex-1 text-[9px] font-bold bg-[#030712] border border-white/10 hover:bg-white/10 text-slate-400 px-2 py-1.5 rounded-lg transition-colors shadow-sm">+60 Hari</button>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-[#030712]/80 backdrop-blur-md rounded-2xl p-6 border border-white/5 relative shadow-xl">
            <h3 className="text-white font-bold mb-4">Identitas Pihak Penawar</h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
              <CleanInput label="Nama Anda / Instansi" name="myName" value={quotationData.myName} onChange={handleQuotationChange} placeholder="Misal: Ahmad Fulan / Kreatif Studio" />
              <CleanInput label="Profesi / Jabatan" name="myRole" value={quotationData.myRole} onChange={handleQuotationChange} placeholder="Misal: Freelance UI/UX Designer" />
              <CleanInput label="Email Aktif" type="email" name="myEmail" value={quotationData.myEmail} onChange={handleQuotationChange} placeholder="Misal: kontak@kreatifstudio.com" />
              <CleanInput label="No. WhatsApp / Telp" name="myPhone" value={quotationData.myPhone} onChange={handleQuotationChange} placeholder="Misal: +62 812 3456 7890" />
            </div>
            <div className="mb-6">
              <CleanInput label="NPWP / Tax ID Pribadi" name="myTaxId" value={quotationData.myTaxId} onChange={handleQuotationChange} placeholder="Misal: 12.345.678.9-012.000" hint="Opsional" />
            </div>

            <div className="h-px w-full bg-white/10 mb-6"></div>

            <h3 className="text-white font-bold mb-4">Identitas Prospek Klien</h3>
            <div className="space-y-4">
              <CleanInput label="Nama Klien / Perusahaan" name="clientName" value={quotationData.clientName} onChange={handleQuotationChange} placeholder="Misal: PT Maju Mundur Sejahtera" />
              <CleanTextarea label="Alamat Klien" name="clientAddress" value={quotationData.clientAddress} onChange={handleQuotationChange} placeholder="Misal: Gedung Cyber Lt. 12, Jl. Jend. Sudirman Kav. 21, Jakarta Selatan 12920" rows={2} />
              <CleanInput label="NPWP / Tax ID Klien" name="clientTaxId" value={quotationData.clientTaxId} onChange={handleQuotationChange} placeholder="Misal: 98.765.432.1-098.000" hint="Opsional" />
            </div>
          </div>

          {/* Card: Items */}
          <div className="bg-[#030712]/80 backdrop-blur-md rounded-2xl p-6 border border-white/5 relative shadow-xl">
            <h3 className="text-white font-bold mb-4">Rincian Estimasi Biaya</h3>
            <div className="space-y-6">
              ...
              <button onClick={addItem} className="w-full py-3.5 rounded-xl border border-dashed border-white/20 text-cyan-400 hover:bg-cyan-500/10 hover:border-cyan-500/50 hover:text-cyan-300 transition-all flex items-center justify-center gap-2 text-sm font-bold bg-[#030712]/50">
                <Icons.Plus /> Tambah Item Estimasi
              </button>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              ...
              <div className="bg-white/5 p-4 rounded-xl border border-white/10">
                 ...
                 <div className="flex mb-3 bg-[#030712] rounded-lg p-1 border border-white/10">
                    <button onClick={() => setQuotationData(prev => ({...prev, discountType: 'fixed'}))} className={`flex-1 text-[10px] py-1.5 rounded-md transition-all font-bold uppercase tracking-widest ${quotationData.discountType === 'fixed' ? 'bg-white/10 text-white shadow-sm' : 'text-slate-500 hover:text-slate-300'}`}>Nominal</button>
                    <button onClick={() => setQuotationData(prev => ({...prev, discountType: 'percent'}))} className={`flex-1 text-[10px] py-1.5 rounded-md transition-all font-bold uppercase tracking-widest ${quotationData.discountType === 'percent' ? 'bg-white/10 text-white shadow-sm' : 'text-slate-500 hover:text-slate-300'}`}>Persen (%)</button>
                 </div>
                 <CleanInput 
                   type="number" 
                   name="discount" 
                   ...
                   onChange={handleQuotationChange} 
                   placeholder={quotationData.discountType === 'fixed' ? "Misal: 50000" : "Misal: 10"} 
                   suffix={quotationData.discountType === 'percent' ? '%' : currency}
                 />
              </div>
              ...
              <div className="bg-white/5 p-4 rounded-xl border border-white/10">
                ...
                <div className="flex flex-wrap gap-2">
                  <button onClick={() => setQuotationData(prev => ({...prev, taxRate: "11"}))} className="text-[10px] font-bold bg-[#030712] border border-white/10 hover:bg-white/10 text-slate-300 px-2.5 py-1.5 rounded-lg transition-colors shadow-sm">PPN 11%</button>
                  <button onClick={() => setQuotationData(prev => ({...prev, taxRate: "12"}))} className="text-[10px] font-bold bg-[#030712] border border-white/10 hover:bg-white/10 text-slate-300 px-2.5 py-1.5 rounded-lg transition-colors shadow-sm">PPN 12%</button>
                  <button onClick={() => setQuotationData(prev => ({...prev, taxRate: "0"}))} className="text-[10px] font-bold bg-[#030712] border border-white/10 hover:bg-white/10 text-slate-300 px-2.5 py-1.5 rounded-lg transition-colors shadow-sm">Tanpa Pajak</button>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-[#030712]/80 backdrop-blur-md rounded-2xl p-6 border border-white/5 relative shadow-xl">
            <h3 className="text-white font-bold mb-4">Opsi Pembayaran (Opsional)</h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
              <div className="sm:col-span-2">
                 <CustomDropdown 
                    label="Bank / Platform Pembayaran" 
                    value={quotationData.bankName} 
                    onChange={(val) => setQuotationData({...quotationData, bankName: val})} 
                    options={bankOptions} 
              ...
              <div className="sm:col-span-2">
                 <CleanInput 
                   ...
                   name="bankCode" 
                   value={quotationData.bankCode} 
                   onChange={handleQuotationChange} 
                   placeholder="Misal: BOCAUS3NXXX" 
                   hint="Opsional (Untuk Global)"
                 />
              </div>
              <CleanInput label="Nomor Akun / Rekening / Email" name="accNumber" value={quotationData.accNumber} onChange={handleQuotationChange} placeholder="Misal: 1234 5678 90" />
              <CleanInput label="Atas Nama (A/N)" name="accName" value={quotationData.accName} onChange={handleQuotationChange} placeholder="Misal: Ahmad Fulan" />
            </div>
            ...
            <div className="space-y-4">
              <CleanTextarea 
                ...
                onChange={handleQuotationChange} 
                placeholder="Misal: Penawaran ini berlaku selama 30 hari. Harga dapat berubah apabila ada penambahan scope pekerjaan di luar rincian di atas." 
                rows={3} 
              />
              <CleanInput 
          ...
          <div className="sticky top-6 w-full flex justify-center h-fit print-reset-layout" ref={previewContainerRef}>
            ...
            >
              ...
              >
                ...
                >
                  {template === 'modern' && <TemplateModern quotationData={quotationData} items={items} logo={logo} stamp={stamp} themeColor={themeColor} currency={currency} subtotal={subtotal} taxAmount={taxAmount} discountAmount={discountAmount} total={total} t={t} />}
                  {template === 'minimal' && <TemplateMinimal quotationData={quotationData} items={items} logo={logo} stamp={stamp} themeColor={themeColor} currency={currency} subtotal={subtotal} taxAmount={taxAmount} discountAmount={discountAmount} total={total} t={t} />}
                  {template === 'classic' && <TemplateClassic quotationData={quotationData} items={items} logo={logo} stamp={stamp} themeColor={themeColor} currency={currency} subtotal={subtotal} taxAmount={taxAmount} discountAmount={discountAmount} total={total} t={t} />}
                </div>
              </div>
            </div>
