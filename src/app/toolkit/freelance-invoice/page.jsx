...
// =========================================================================
// KOMPONEN IKON SVG MINIMALIS & PREMIUM
// =========================================================================
...
// Merubah warna HEX kembali ke koordinat Hue (0-360) agar slider bisa menyesuaikan posisi
const hexToHue = (hex) => {
  let r = 0, g = 0, b = 0;
  if (hex.length === 4) {
...
// =========================================================================
// KOMPONEN INPUT FORM REUSABLE (Diletakkan di luar agar Keyboard tidak hilang)
// =========================================================================
...
const CustomColorSlider = ({ color, onChange }) => {
  const hue = hexToHue(color) || 0; // Mengambil koordinat slider dari warna saat ini

  return (
    <div className="w-full flex flex-col gap-3">
      {/* Slider Visual Anti-Popup */}
      <div className="relative w-full h-8 rounded-xl overflow-hidden border border-white/10 shadow-inner" style={{ background: 'linear-gradient(to right, #ff0000, #ffff00, #00ff00, #00ffff, #0000ff, #ff00ff, #ff0000)' }}>
        ...
        {/* Indikator Pemilih Warna (Thumb) */}
        <div 
          className="absolute top-0 bottom-0 w-4 bg-white border-2 border-slate-900 rounded-md shadow-md pointer-events-none transition-transform duration-75"
          style={{ left: `clamp(0px, calc(${(hue / 360) * 100}% - 8px), calc(100% - 16px))` }}
        />
        
        {/* Input asli (tersembunyi tapi bisa digeser) */}
        <input 
...
export default function InvoiceStudio() {
  ...
  // ==========================================
  // STATE: INVOICE GENERATOR (Kosong by default)
  // ==========================================
  const [invoiceData, setInvoiceData] = useState({
    ...
    discount: "",
    notes: "",
    signatureName: "", // Fitur Baru Tanda Tangan
    status: "UNPAID", 
  });

  // Efek pengisian tanggal otomatis (agar aman dari error Hydration Next.js)
  useEffect(() => {
            ...
            <div className="lg:col-span-5 space-y-6 print:hidden">
              ...
              <div className="bg-[#0a0a0a] border border-white/5 rounded-[2rem] p-6 sm:p-8 shadow-xl">
                ...
                <div className="space-y-4 mt-4">
                  <CleanInput label="Nama Penanda Tangan" name="signatureName" value={invoiceData.signatureName} onChange={handleInvoiceChange} placeholder="Nama Tanda Tangan Anda" prefix={<IconSignature />} />
                  <div>
            ...
            <div className="lg:col-span-7 print:col-span-12 w-full">
              <div className="w-full overflow-x-auto bg-[#0a0a0a] rounded-[2rem] p-4 md:p-8 custom-scroll border border-white/5 print:border-none print:p-0 print:bg-white">
                ...
                <div className="w-[794px] min-h-[1123px] mx-auto bg-white text-slate-900 shadow-2xl print:w-full print:min-h-0 print:shadow-none flex flex-col relative overflow-hidden transition-all duration-500">
                  ...
                  {/* WATERMARK STAMP UNTUK STATUS (PAID/UNPAID) */}
                  ...
                  <div className={`flex-1 flex flex-col z-10 ${template === 4 ? 'pl-10' : ''}`}>
                    ...
                    <div className="px-12 flex-1 flex flex-col">
                      ...
                      {/* AREA TANDA TANGAN & FOOTER (FITUR BARU) */}
