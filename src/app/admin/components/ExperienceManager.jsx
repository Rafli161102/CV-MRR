...
export default function ExperienceManager({ token }) {
  ...
  const fetchItems = async () => {
    try {
      setIsLoading(true);
      const response = await fetch('/api/cms/work-experience');
  ...
  const handleAddNew = () => {
    ...
    setFormData({ year: '', role: '', company: '', description: '' });
    setShowForm(true);
  };
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const response = await fetch(`/api/cms/work-experience?id=${id}`, {
        method: 'DELETE',
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!formData.year || !formData.role || !formData.company) {
