...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-[#030712] text-slate-300 font-sans selection:bg-cyan-500 selection:text-white relative overflow-hidden">
      ...
      {/* ========================================================= */}
      {/* BACKGROUND EFFECTS (Elegan & Dalam) */}
      {/* ========================================================= */}
      <div className="fixed inset-0 pointer-events-none z-0">
        <div className="absolute top-[-10%] left-[-10%] w-[50rem] h-[50rem] bg-cyan-600/10 rounded-full blur-[120px]"></div>
        <div className="absolute bottom-[-20%] right-[-10%] w-[40rem] h-[40rem] bg-indigo-600/10 rounded-full blur-[150px]"></div>
        {/* Grid pattern overlay untuk aksen UI/UX */}
        <div className="absolute inset-0 bg-[url('https://grainy-gradients.vercel.app/noise.svg')] opacity-[0.02] mix-blend-overlay"></div>
        {/* Garis Vertikal Halus (Menciptakan kesan Grid System) */}
        <div className="absolute left-[10%] top-0 bottom-0 w-[1px] bg-white/[0.02]"></div>
        <div className="absolute left-[50%] top-0 bottom-0 w-[1px] bg-white/[0.02]"></div>
        <div className="absolute right-[10%] top-0 bottom-0 w-[1px] bg-white/[0.02]"></div>
      </div>
      ...
      <div className="relative z-10 pt-32 pb-24">
        <div className="max-w-[1400px] mx-auto px-6 lg:px-12">
          ...
          {/* ========================================================= */}
          <div className="flex flex-col lg:flex-row items-center justify-between gap-16 lg:gap-0 min-h-[75vh]">
            ...
            <div className="w-full lg:w-[61.8%] flex flex-col justify-center text-center lg:text-left pr-0 lg:pr-12 z-20">
              ...
              <div className="anim-fade-in-up inline-flex items-center justify-center lg:justify-start gap-3 mb-6">
                ...
                <span className="text-slate-300 text-xs sm:text-sm font-semibold tracking-wide border border-white/10 px-4 py-1.5 rounded-full bg-white/5 backdrop-blur-sm">
                  Design Graphic & Community Development
                </span>
              </div>
              
              {/* Headline dengan kontras tinggi ala Dribbble/Awwwards */}
              <h1 className="anim-fade-in-up anim-delay-100 text-[2.75rem] sm:text-6xl lg:text-[5rem] font-black tracking-tighter text-white mb-6 leading-[1.1] drop-shadow-lg">
              ...
              <p className="anim-fade-in-up anim-delay-200 text-base sm:text-lg text-slate-400 font-medium leading-relaxed mb-10 max-w-xl mx-auto lg:mx-0">
              ...
              <div className="anim-fade-in-up anim-delay-300 flex flex-col sm:flex-row items-center gap-4 justify-center lg:justify-start">
                
                {/* Tombol Lihat & Download CV Pribadi */}
                <Link 
                  href="/cv-print" 
            ...
            <div className="w-full lg:w-[38.2%] flex justify-center lg:justify-end relative mt-12 lg:mt-0 z-10">
              
              {/* Desain bingkai UI Modern (Glassmorphism + Floating Elements) */}
              <div className="relative w-72 h-[22rem] sm:w-80 sm:h-[26rem] lg:w-[22rem] lg:h-[28rem] group anim-float">
                ...
                {/* Floating Badge Kiri (Komunitas) */}
                <div className="absolute -bottom-6 -left-6 lg:-left-12 z-20 bg-[#0A1329]/90 backdrop-blur-xl p-4 sm:p-5 rounded-2xl border border-white/10 shadow-[0_20px_40px_rgba(0,0,0,0.6)] hover:-translate-y-2 transition-transform duration-300">
                  <div className="text-[9px] sm:text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em] mb-1">Founder of</div>
                ...
                <div className="absolute -top-6 -right-6 lg:-right-8 z-20 bg-gradient-to-br from-cyan-600 to-blue-700 p-3 sm:p-4 rounded-2xl shadow-[0_10px_30px_rgba(6,182,212,0.4)] hover:scale-105 transition-transform duration-300 flex items-center gap-3">
                  <div className="text-2xl sm:text-3xl font-black text-white">3+</div>
                  <div className="text-[9px] sm:text-[10px] font-bold text-cyan-100 uppercase tracking-wider leading-tight">Years<br/>Experience</div>
                </div>
          ...
          <div className="mt-32 w-full anim-fade-in-up anim-delay-300">
            <div className="relative rounded-3xl overflow-hidden border border-white/10 bg-gradient-to-r from-[#0a152e] to-[#050b1a] p-8 md:p-10 flex flex-col md:flex-row items-center justify-between gap-8 shadow-2xl group">
              ...
              <div className="flex items-start gap-5 z-10 text-center md:text-left">
                <div className="p-3 bg-cyan-900/30 border border-cyan-500/30 rounded-2xl hidden md:block group-hover:scale-110 transition-transform duration-500">
                  <ToolsIcon />
                </div>
                <div>
                  <div className="text-cyan-400 text-[10px] font-bold uppercase tracking-[0.2em] mb-2">Micro-SaaS Buatan Saya</div>
                  <h3 className="text-2xl md:text-3xl font-black text-white mb-3 tracking-tight">HRD-Ready ATS CV Maker</h3>
              ...
              <Link href="/cv-maker" className="z-10 whitespace-nowrap px-8 py-4 bg-cyan-600 hover:bg-cyan-500 text-white font-bold rounded-xl transition-all shadow-[0_0_20px_rgba(6,182,212,0.3)] hover:shadow-[0_0_30px_rgba(6,182,212,0.5)] flex items-center gap-3 hover:-translate-x-1">
                Coba Tools Gratis
                <ArrowRightIcon />
              </Link>
            </div>
          ...
          {/* ========================================================= */}
          {/* BENTO GRID: KARYA TERPILIH (Desain Asimetris & Modern)    */}
          {/* ========================================================= */}
            ...
            <div className="flex flex-col sm:flex-row justify-between items-end mb-12 gap-6">
              <div>
                <div className="flex items-center gap-2 mb-3">
                  <span className="w-4 h-4 bg-cyan-500 rounded-sm rotate-45"></span>
                  <span className="text-cyan-400 font-bold uppercase tracking-widest text-xs">Portofolio</span>
                </div>
                <h2 className="text-3xl sm:text-4xl font-black text-white">Pilihan <span className="text-cyan-500">Mahakarya</span></h2>
              </div>
              <Link href="/projects" className="text-cyan-400 hover:text-cyan-300 font-bold tracking-widest uppercase text-sm group flex items-center gap-2 transition-all pb-1">
                Jelajahi Galeri 
                <span className="bg-white/10 p-1.5 rounded-full group-hover:bg-cyan-500 group-hover:text-white transition-colors">
                  <ArrowRightIcon />
                </span>
              </Link>
            </div>

            {/* Layout Bento Grid Sejati (1 Utama Besar, 2 Pendukung) */}
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 auto-rows-[350px] lg:auto-rows-[450px]">
              {featuredProjects.map((project, index) => (
                <Link 
                  href={`/projects/${project.id}`} 
                  key={project.id} 
                  // Logika Bento: Item index 0 memakan 2 kolom, sisanya 1 kolom
                  className={`group relative rounded-[2rem] overflow-hidden bg-[#0A1329] border border-white/5 hover:border-cyan-500/40 transition-all duration-500 shadow-lg hover:shadow-[0_20px_50px_rgba(6,182,212,0.15)] 
                ...
                >
                  ...
                  <div className="absolute bottom-0 left-0 w-full p-8 lg:p-10 flex flex-col justify-end transform translate-y-4 group-hover:translate-y-0 transition-transform duration-500">
                    <div className="inline-flex items-center gap-2 text-[10px] font-bold tracking-[0.2em] text-cyan-400 uppercase mb-3 bg-cyan-900/40 backdrop-blur-md border border-cyan-500/30 px-3 py-1.5 rounded-full w-fit">
                      <span className="w-1.5 h-1.5 rounded-full bg-cyan-400"></span>
                      {project.category}
                    </div>
                    <h3 className={`font-bold text-white group-hover:text-cyan-300 transition-colors tracking-tight ${index === 0 ? 'text-3xl md:text-4xl lg:text-5xl' : 'text-2xl lg:text-3xl'}`}>
                      {project.title}
                    </h3>
                  </div>
                </Link>
