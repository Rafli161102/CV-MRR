  ...
  return (
    <div className="show-in-pwa fixed bottom-0 left-0 w-full z-50">
      ...
      <div className="bg-[#03060a]/95 backdrop-blur-2xl border-t border-white/10 pb-[env(safe-area-inset-bottom)] shadow-[0_-15px_40px_rgba(0,0,0,0.8)] relative">
        ...
        <div className="flex justify-around items-end h-[68px] px-4 max-w-md mx-auto pb-1.5">
          ...
          <Link href="/css-studio" className="group flex flex-col items-center justify-center relative w-20 scale-100 active:scale-95 transition-all duration-300 -mt-5">
            
            {/* Ambient Glow Khusus Engine */}
            <div className={`absolute top-0 rounded-full blur-[15px] w-12 h-12 -z-10 transition-opacity duration-500 ${pathname === '/css-studio' ? 'bg-cyan-500 opacity-30 animate-pulse' : 'bg-transparent opacity-0'}`}></div>

            {/* Sci-Fi Center Button (Hexagon Shape) */}
            <div className={`flex items-center justify-center w-[52px] h-[52px] relative z-10 transition-all duration-300 ${
              pathname === '/css-studio' 
